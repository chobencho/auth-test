...
export const UserEdit = ({ currentUser }) => {
  ...
  const handleSaveUserData = async (e) => {
    // axiosInstance.post(`/users/${params.id}/update`, { name, body, age, gender, prefecture, grade, subject });
    axiosInstance.post(`/users/${params.id}/update`, { name, body, age, gender, prefecture, grade, subject });
  ...
  return (
    ...
    <div class="w-11/12 mx-auto py-20">
      ...
      <div class="w-full">
        ...
        <textarea name="" id="" cols="30" rows="5" class="border border-gray-200 rounded w-full mb-3" value={body} key={userData?.body} defaultValue={userData?.body} onChange={(e) => setBody(e.target.value)} ></textarea>
        ...
        <select class="border border-gray-200 rounded w-full mb-3 h-8" value={prefecture} key={userData?.prefecture_code} defaultValue={userData?.prefecture_code} onChange={(e) => setPrefecture(e.target.value)}>
